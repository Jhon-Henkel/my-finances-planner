...
describe('Testing the future gain form render', () => {
    it('should render form for insert one future gain', async () => {
        ...
        const wrapper = shallowMount(FutureGainForm, {
            global: {
                stubs: {
        ...
        expect(wrapper.html()).toContain('customclassrow="justify-content-center" customclassform="mt-2" showtitle="true" title="Valor" masked="false" precision="2" decimal="," thousands="." suffix="" usefloatinglabels="true" value="0"></input-money-stub>')
        expect(wrapper.html()).toContain('class="row justify-content-center mt-3">')
        expect(wrapper.html()).toContain('class="form-check-label" for="fix-gain"> Despesa fixa </label>')
        expect(wrapper.html()).toContain('type="checkbox" role="switch" id="fix-gain"></div>')
        ...
        expect(wrapper.html()).toContain('redirectto="/ganhos-futuros" buttonsuccesstext="" buttonsuccessicon="fas,check" buttoncanceltext="Voltar" buttoncancelicon="fas,angle-left" showbuttoncancel="true"></bottom-buttons-stub>')
    })
})